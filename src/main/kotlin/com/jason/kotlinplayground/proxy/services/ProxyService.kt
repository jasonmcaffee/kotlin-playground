...
@Service
class ProxyService(private val cachedResponseEntityRepository: CachedResponseEntityRepository) {
    suspend fun proxyRequest(urlToProxyTo: String, body: String?, method: HttpMethod, request: HttpServletRequest, response: HttpServletResponse): ResponseEntity<String>{
        ...
        val cachedResponseEntity = io { cachedResponseEntityRepository.findCachedResponseBy(method.toString(), urlToProxyTo, body) }
        ...
        io { cachedResponseEntityRepository.save(cachedResponse) }
        return result
    }
}
